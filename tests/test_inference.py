...
class TestInference(unittest.TestCase):
    ...
    def test_esm_inference_from_chain(self):
        ...
        esm_embeddings = predict(
            src_stream=[
                ("8GYM", f"{self.__test_path}/resources/8GYM.cif", "A", "8GYM.A"),
        ...
        self.assertEqual(tuple(esm_embeddings[0][0][0].shape), (72, 1536))
        self.assertEqual(tuple(esm_embeddings[1][0][0].shape), (439, 1536))
    ...
    def test_esm_inference_from_structure(self):
        ...
        esm_embeddings = predict(
            src_stream=[
                ("1acb", f"{self.__test_path}/resources/1acb.cif", "1acb"),
    ...
    def test_chain_inference_from_tensor_files(self):
        ...
        chain_embeddings = predict(
            src_stream=[
                (f"{self.__test_path}/resources/1acb.A.pt", "1acb.A"),
                (f"{self.__test_path}/resources/1acb.B.pt", "1acb.B"),
                (f"{self.__test_path}/resources/2uzi.A.pt", "2uzi.A"),
                (f"{self.__test_path}/resources/2uzi.B.pt", "2uzi.B"),
                (f"{self.__test_path}/resources/2uzi.C.pt", "2uzi.C")
            ],
            src_location=SrcLocation.stream,
            accelerator=Accelerator.cpu
        )
